...
/** POST /api/admin/host — profile + destination/conflict calendar selection. */
export async function POST(req: Request) {
  ...
  try {
    ...
    log.info("admin", "host_updated");
    return ok({ saved: true });
  } catch (err) {
    if (err instanceof ValidationError) return fail(err.message, 400, "VALIDATION");
    log.error("admin", "host_update_failed", { error: errorMessage(err) });
