...
/** A mail send that returned false (provider down / not configured) must retry. */
async function mustSend(p: Promise<boolean>, what: string) {
  if (!(await p)) throw new Error(`${what}: mail not sent (provider error or not configured)`);
}
...
registerJobHandler("reminder", async (job) => {
  const ctx = await ctxOrStop(job);
